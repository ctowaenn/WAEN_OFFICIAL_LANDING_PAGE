...
| Valor | Descripción |
|--------|-------------|
| **`waenn-proxy`** (predeterminado en [`index.html`](../index.html)) | Módulo + `POST /api/subscribe` cuando la página se sirve por **http(s)**. El servidor llama a la API **Double Opt-In** de Brevo (`/v3/contacts/doubleOptinConfirmation`). El cliente solo muestra éxito si la respuesta JSON incluye **`{ "ok": true }`**. **No hay fallback** a Sibforms. Requiere env vars en Vercel (ver más abajo). Con **`file://`** no existe `/api/subscribe`; usa `vercel dev` o cambia temporalmente a `waenn-sibforms`. La API key solo en el servidor. |
| **`waenn-sibforms`** | Mismo módulo; el navegador envía al `action` HTTPS de `*.sibforms.com` obtenido solo desde [`iframe.html`](../iframe.html). Sin backend propio; menos fiable que el proxy para confirmar envíos. Tras republicar el formulario en Brevo, actualiza `iframe.html` y el export en [`brevo.html`](../brevo.html). |
